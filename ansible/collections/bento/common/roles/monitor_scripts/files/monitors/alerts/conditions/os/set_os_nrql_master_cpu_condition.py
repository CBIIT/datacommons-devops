...
def setcondition(key, project, tier, policy_id):
   ...
   condition_name = '{} {} Opensearch Master CPU Utilization'.format(project, tier)
   query = "FROM  Metric SELECT average(`aws.es.MasterCPUUtilization`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'"
   critical_duration = "5"
