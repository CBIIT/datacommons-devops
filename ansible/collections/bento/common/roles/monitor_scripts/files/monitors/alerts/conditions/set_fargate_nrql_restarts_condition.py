...
def setfargaterestartscondition(key, project, tier, policy_id):
   ...
   condition_name = '{}-{} Fargate Task Restarts'.format(project.title(), tier.title())
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM ContainerSample SELECT max(restartCount) - min(restartCount) WHERE ecsClusterName LIKE '{}-{}-%'".format(project.lower(), tier.lower())
       },
       "signal" : {
