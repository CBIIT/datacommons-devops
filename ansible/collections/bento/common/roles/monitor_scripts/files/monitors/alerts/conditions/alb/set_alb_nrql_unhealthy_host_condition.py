...
def setcondition(key, project, tier, policy_id):
   ...
   query = "FROM Metric SELECT average(`aws.applicationelb.UnHealthyHostCount`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ApplicationELB' AND entity.name LIKE '%{}-{}-lb%'"
   critical_duration = "5"
