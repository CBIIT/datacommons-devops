...
def setcondition(key, project, tier, policy_id):
   ...
   query = "FROM Metric SELECT sum(`aws.applicationelb.HTTPCode_ELB_5XX_Count`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ApplicationELB' AND entity.name LIKE '%{}-{}-lb%'"
   critical_duration = "5"
