...
def setcondition(key, project, tier, policy_id):
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM  Metric SELECT min(`aws.es.ClusterStatus.yellow`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'".format(project.lower(), tier.lower())
       },
       "signal" : {
