...
def setcondition(key, project, tier, policy_id):
   ...
   condition_name = '{} {} ALB TLS Errors'.format(project, tier)
   query = "FROM Metric SELECT sum(`aws.applicationelb.TargetTLSNegotiationErrorCount`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ApplicationELB' AND entity.name LIKE '%{}-{}-lb%'"
   critical_duration = "5"
