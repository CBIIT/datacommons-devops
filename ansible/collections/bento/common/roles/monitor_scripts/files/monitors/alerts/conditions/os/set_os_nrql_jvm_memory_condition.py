...
def setcondition(key, project, tier, policy_id):
   ...
   condition_name = '{}-{} Opensearch JVM Memory Usage'.format(project.title(), tier.title())
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM  Metric SELECT average(`aws.es.JVMMemoryPressure.byCluster`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'".format(project.lower(), tier.lower())
       },
       "signal" : {
