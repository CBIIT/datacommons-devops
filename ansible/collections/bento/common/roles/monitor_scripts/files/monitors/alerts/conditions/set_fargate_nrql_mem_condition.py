...
def setfargatememcondition(key, project, tier, policy_id):

   API_ENDPOINT = 'https://api.newrelic.com/v2/alerts_nrql_conditions.json'

   condition_name = '{}-{} Fargate Memory Usage'.format(project.title(), tier.title())
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM Metric SELECT average(`aws.ecs.MemoryUtilization.byService`) as 'Memory utilization (%)' WHERE aws.ecs.ClusterName LIKE '{}-{}-%'".format(project.lower(), tier.lower())
       },
       "signal" : {
