...
def setalbhostcondition(key, project, tier, policy_id):

   API_ENDPOINT = 'https://api.newrelic.com/v2/alerts_nrql_conditions.json'

   condition_name = '{}-{} ALB Unhealthy Hosts'.format(project.title(), tier.title())
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM Metric SELECT average(`aws.applicationelb.UnHealthyHostCount`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ApplicationELB' AND entity.name LIKE '%{}-{}-lb%'".format(project.lower(), tier.lower())
       },
       "signal" : {
