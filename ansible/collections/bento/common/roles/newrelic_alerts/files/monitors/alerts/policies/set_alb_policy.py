...
def setalbalertpolicy(project, tier, key):
   ...
   set_alb_nrql_elb_4xx_condition.setalbelb4xxcondition(key, project, tier, policy_id)

   # add alb elb 5xx condition
   set_alb_nrql_elb_5xx_condition.setalbelb5xxcondition(key, project, tier, policy_id)

   # add alb target 5xx condition
   set_alb_nrql_target_5xx_condition.setalbtarget5xxcondition(key, project, tier, policy_id)

   # add alb target response condition
   set_alb_nrql_target_response_condition.setalbtargetcondition(key, project, tier, policy_id)
   
   # add alb tls condition
   set_alb_nrql_tls_condition.setalbtlscondition(key, project, tier, policy_id)

   # add alb unhealthy host condition
   set_alb_nrql_unhealthy_host_condition.setalbhostcondition(key, project, tier, policy_id)
