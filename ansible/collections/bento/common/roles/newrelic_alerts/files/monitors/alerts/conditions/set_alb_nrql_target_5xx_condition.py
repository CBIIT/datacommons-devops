...
def setalbtarget5xxcondition(key, project, tier, policy_id):

   API_ENDPOINT = 'https://api.newrelic.com/v2/alerts_nrql_conditions.json'

   condition_name = '{}-{} ALB Target 5xx Count'.format(project.title(), tier.title())
   ...
   data = {
     "nrql_condition" : {
       ...
       "nrql" : {
         "query" : "FROM Metric SELECT sum(`aws.applicationelb.HTTPCode_Target_5XX_Count`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ApplicationELB' AND entity.name LIKE '%{}-{}-lb%'".format(project.lower(), tier.lower())
       },
       "signal" : {
