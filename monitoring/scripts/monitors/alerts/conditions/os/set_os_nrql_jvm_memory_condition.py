...
def setcondition(key, project, tier, policy_id):
   ...
   query = "FROM  Metric SELECT average(`aws.es.JVMMemoryPressure.byCluster`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'"
   critical_duration = "3"
