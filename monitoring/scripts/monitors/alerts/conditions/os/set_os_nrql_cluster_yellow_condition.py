...
def setcondition(key, project, tier, policy_id):
   ...
   query = "FROM  Metric SELECT min(`aws.es.ClusterStatus.yellow`) WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'"
   warning_duration = "1"
   warning_threshold = "0"
