...
def setcondition(key, project, tier, policy_id):
   ...
   query = "FROM  Metric SELECT (sum(`aws.es.5xx`) > (sum(`aws.es.OpenSearchRequests`) * 0.01)) as `5xx` WHERE collector.name='cloudwatch-metric-streams' AND aws.Namespace='AWS/ES' AND entity.name = '{}-{}-opensearch'"
   critical_duration = "5"
