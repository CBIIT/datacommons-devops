#!/usr/bin/python
"""
DataDog metric alert monitor: ALB ELB 4xx error count.

Replaces: New Relic NRQL condition
  FROM Metric SELECT sum(aws.applicationelb.HTTPCode_ELB_4XX_Count)
  WHERE entity.name LIKE '%{project}-{tier}-lb%'
  > 0 (critical, 5 min)

DataDog metric: aws.applicationelb.httpcode_elb_4xx
  Sourced from the DataDog AWS CloudWatch integration for ALB.
  Tag filter: loadbalancer scoped to the project+tier load balancer.
"""

from monitors import dd_client


def setmonitor(project, tier, notification):
    monitor_name = "{} {} ALB ELB 4xx Count".format(project, tier)
    lb_filter = "loadbalancer:app/*{project}-{tier}*".format(
        project=project.lower(), tier=tier.lower()
    )

    payload = {
        "name": monitor_name,
        "type": "metric alert",
        "query": "sum(last_5m):sum:aws.applicationelb.httpcode_elb_4xx{{{filter}}}.as_count() > 0".format(
            filter=lb_filter
        ),
        "message": (
            "ALB is returning 4xx errors for {project} {tier}.\n{notification}"
        ).format(project=project, tier=tier, notification=notification),
        "tags": [
            "project:{}".format(project.lower()),
            "tier:{}".format(tier.lower()),
        ],
        "priority": 2,
        "options": {
            "thresholds": {
                "critical": 0,
            },
            "notify_no_data": False,
            "evaluation_delay": 120,
            "include_tags": True,
        },
    }

    monitor_id = dd_client.find_monitor(monitor_name)
    if monitor_id:
        print("{} already exists, updating with latest configuration.".format(monitor_name))
    else:
        print("{} not found, creating.".format(monitor_name))

    monitor_id = dd_client.upsert_monitor(monitor_id, payload)
    print("{} upserted (id: {}).".format(monitor_name, monitor_id))
    return monitor_id
