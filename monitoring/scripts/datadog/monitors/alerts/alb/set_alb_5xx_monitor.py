#!/usr/bin/python
"""
DataDog metric alert monitor: ALB ELB 5xx error count.

Replaces: New Relic NRQL condition
  FROM Metric SELECT sum(aws.applicationelb.HTTPCode_ELB_5XX_Count)
  WHERE entity.name LIKE '%{project}-{tier}-lb%'
  > 1 (critical, 5 min)

DataDog metric: aws.applicationelb.httpcode_elb_5xx
"""
...
def setmonitor(project, tier, notification):
    monitor_name = "{} {} ALB ELB 5xx Count".format(project, tier)
    ...
    payload = {
        "name": monitor_name,
        "type": "metric alert",
        "query": "sum(last_5m):sum:aws.applicationelb.httpcode_elb_5xx{{{filter}}}.as_count() > 1".format(
            filter=lb_filter
        ),
        "message": (
            "ALB is returning 5xx errors for {project} {tier}.\n{notification}"
